...
1. **🔌 유튜브 API 키 불필요 (무인증 검색)**
   ...
   - CORS 차단 상황에서도 corsproxy.io → allorigins.win 순서의 다중 CORS 프록시 폴백을 내장하고 있습니다.
...
5. **🚫 임베드 차단 영상 자동 필터링**
   - 예약 시 YouTube oEmbed API로 임베드 가능 여부를 사전 확인하고, 차단된 영상은 자동으로 건너뛰고 다음 후보 영상을 탐색합니다.
   - 재생 중 차단 에러(101/150) 발생 시 화면 하단 토스트 알림을 표시하고 다음 대기곡으로 자동 스킵합니다.

6. **🎙️ 커버곡 제외 & 노래방 MR 반주 우선 정렬**
   - 검색 결과에서 일반인 커버(cover), 라이브, 뮤직비디오 등 보컬 영상을 점수 기반으로 자동 제거합니다.
   - `노래방`, `반주`, `MR`, `karaoke`, `instrumental` 키워드가 있는 영상에 가산점을 부여해 반주 영상을 최상단에 우선 노출합니다.
   - **TJ/KY/금영 공식 채널 영상은 외부 임베드가 전부 차단**되어 있으므로 검색 결과에서 자동 제외합니다.
   - 4단계 검색 전략으로 반주 영상을 최우선 탐색합니다: `[번호] MR` → `노래방 반주` → `가수 MR` → 브로드 폴백

7. **🎨 사이버펑크 네온 & CRT 모니터 효과**
...
*   [개발 완료 보고서 (Doc/walkthrough.md)](./Doc/walkthrough.md) - 컴포넌트 구조 설명, 빌드/배포 검증 결과, 임베드 필터·커버곡 필터 시스템 상세
*   [태스크 리스트 (Doc/task.md)](./Doc/task.md) - 전체 개발 작업 이력 체크리스트
