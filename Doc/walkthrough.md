...
8. **커버곡 제외 및 노래방 MR 반주 영상 우선 정렬**:
   ...
   - `노래방`, `반주`, `MR`, `karaoke`, `instrumental` 등 반주 관련 키워드가 포함된 영상에 높은 점수를 부여하여 목록 최상단에 정렬합니다.
   - **TJ/KY/금영 공식 채널 영상은 외부 임베드가 전부 차단**되어 있으므로 점수 -100 페널티로 검색 결과에서 완전 제외합니다.
...
| 분류 | 키워드 예시 | 점수 |
|------|------------|------|
| **MR/반주 긍정 키워드** | `노래방`, `반주`, `MR`, `karaoke`, `instrumental` | +10점 |
| **TJ/KY/금영 공식 채널** | 채널명이 `TJ미디어`, `TJ Media`, `금영`, `Kumyoung` 등 | **-100점 (검색 결과에서 완전 제외)** |
| **커버/보컬 부정 키워드** | `커버`, `cover`, `라이브`, `live`, `MV`, `직캠`, `부른`, `가창` | -20점 |

- TJ/KY/금영 공식 채널 영상은 외부 재생(Embed)이 전부 차단되어 있어 **검색 단계에서부터 완전 제외**
